...
const Router = {
  ...
  goAuth(authView) {
    ...
    /* When entering the reset-password view, always start
       at phase 1 (OTP) so the two-step flow resets cleanly. */
      ...
      .map(
        error => {
          ...
          const friendly = {
            ...
            password_reset_by_code_invalid:
              'The reset code is invalid or has expired.',
    ...
    async () => {
      ...
      try {
        ...
        if (verifyRes.status === 200) {
          ...
          /* ── Wrong code (400 token_invalid) or other error ── */
  ...
  ?.addEventListener(
    ...
    async () => {
      ...
      try {
        ...
        /* ====================================================
           SINGLE STEP — VALIDATE OTP + SET NEW PASSWORD
           ====================================================
           POST /auth/password/reset validates the key and
           changes the password in one request.
           X-Session-Token is sent automatically by apiFetch
           (stored from the 401 returned by /password/request).
           ==================================================== */
         ...
         * IMPORTANT:
         *
         * resetRes.ok can be true even when the underlying
         * HTTP response was 401.
         *
         * api.js normalizes:
         *
         * 200 → successful reset
         * 401 → successful reset, login required
         */

        if (
          resetRes.ok &&
          resetRes.passwordReset
        ) {

          /*
           * Preserve email before clearing State.
           */
          ...
          /* =================================================
             CASE 1:
             ALLAUTH AUTOMATICALLY LOGGED USER IN
             ================================================= */

          if (
            resetRes.authenticated
          ) {
            ...
            /* ===============================================
               CASE 2:
               PASSWORD CHANGED BUT LOGIN REQUIRED
               =============================================== */
        ...
        if (isConflict) {
          ...
          /* Pending flow lost (session expired or never started).
             Clear everything and send user back to forgot-password. */
          ...
          /* Wrong OTP code — go back to phase 1 so user can re-enter */
...
/*
 * Your supplied api.js does not contain a loginWithGoogle()
 * implementation, while your supplied app.js calls it.
 *
 * Do not invent an endpoint here.
 *
 * If your project already defines loginWithGoogle() in another
 * JavaScript file, that implementation can remain there.
 */
...
async function handleGoogleLogin(
  ...
  try {
    ...
    if (
      typeof API.auth.loginWithGoogle !==
      'function'
    ) {
...
async function handleGoogleLogin(alertContainerId) {
  ...
  try {
    ...
    /* ── User closed the popup / One Tap was suppressed ── */
    ...
    /* ── Pending: new Google user needs to complete signup ── */
    if (result?.pendingSignup) {

      Toast.show(
        'Almost there! Please check your details to complete sign-up.',
        'info',
        5000
      );

      // allauth will handle provider_signup flow automatically
      // when the user lands on the verify-email screen if needed.
      // For now, surface a clear message.
      if (alertContainerId) {
        showAlert(
          alertContainerId,
          'Your Google account is new here. Please complete sign-up.'
        );
      }
